...
print(f'LINE32 beginning of file has run')
...
#             print(f'Worker f terminated.')
#             f.join()
#             break

#         # data_image = pre.matplotlib_table_map_generate(sum(v))
#         # import cv2
#         # data_image = cv2.imread(v)
#         # pre.matplotlib_table_map_generate(data_image)
#         print(f'LINE43 aaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaa={v}')
#         # cv2.imwrite('image_multiprocessing_test.png', data_image)
#         # Upload to wandb if available
#         # Cannot do in process not initialized in...so send it back
#         # wandb.Image({'Map': data_image})
#         print(f'LINE50 input received={v}')
#         data_image = v
#         output_queue.put(data_image)
#         print(f'data sent from f: {data_image}')
#         # plt.close('all')
...
def init(
    ...
    if track:
        ...
        # worker_process = Process(target=f, args=(input_queue, output_queue))
...
def evaluate(data):
    ...
    data.stats = {}
    print(f'LINE370 has executed')
    print(f'{infos["learner"].items()}\n\n')
    print(f'INFOS.items()={infos.items()}\n\n')
    print(f'new heatmap in infos? : {infos["heatmap"]}')
    print(f'new heatmap_2 in infos? : {infos["heatmap_2"]}')


    for k, v in infos["learner"].items():
        if 'pokemon_exploration_map' in k:
            print(f'LINE 356 aaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaa={v}')
            # data.input_queue.put(v)
            # data.input_queue.put('put from LINE 370')
            # data_image = data.output_queue.get(c)
            # print(f'received from worker f: {data_image}')
            # data.input_queue.put('kanto_map_dsv.png')
            # data_image = data.output_queue.get(c)
            
            if data.wandb is not None:
                data.stats['Media/exploration_map'] = data.wandb.Image(data_image)
            # data.wandb.Image({'Map': data_image})
        else:
            data_image = None
...
def train(data):
    ...
    # infos = defaultdict(lambda: defaultdict(list))
    # data_image = None
    if data.update % 2 == 0 or done_training:
        # Jerry-rigged logging cuz lazy
        # Needs to write to file so it can be read when dashboard overlay is generated by pokemon_red_eval
        # I realize this is atrocious coding
        ...
        # for k, v in infos['learner'].items():
        #     if 'pokemon_exploration_map' in k:
        #         print(f'__line__ LINE 617 aaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaa={v}')
